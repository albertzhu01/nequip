...
for i in range(50):
    ...
    for j in range(num_atoms):
        for k in range(num_atoms):
            atom_inf_feat = hidden_features[j][0:32]
            atom_val_feat = concat_data_list[k][i][0:32]
...
sns.heatmap(df_euc_dists, mask=mask, square=True, cmap='YlGnBu', vmin=1, vmax=3.1)
plt.title('Average Distance between Inference and Validation Features 1-32 Epoch 50')
